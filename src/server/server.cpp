...
int main(const int argc, const char *argv[]) {
  ...
  if (!cert.empty() && pem.empty()) {
    std::cerr
        << "ERROR: Provided certificate but no matching private key given..."
        << std::endl;
    std::cerr << args.usage();
    exit(0);
  }
  ...
  if (!cert.empty() && !pem.empty()) {
    ...
    if (SSL_CTX_use_certificate_file(ctx.get(), cert.c_str(),
                                     SSL_FILETYPE_PEM) <= 0) {
      server::ssl_errors("[!] Failed to load cert file... exiting");
    }
    if (SSL_CTX_use_PrivateKey_file(ctx.get(), pem.c_str(), SSL_FILETYPE_PEM) <=
        0) {
  ...
  auto listenBio = server::UniquePtr<BIO>(
      BIO_new_accept(fmt::format("{}:{}", ip, port).c_str()));
  if (BIO_do_accept(listenBio.get()) <= 0) {
    server::ssl_errors(
        fmt::format("FATAL: Could not bind to {} on port {}... exiting", ip,
                    port)
            .c_str());
  }
