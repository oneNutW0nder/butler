...
const std::string SERVER_ROOT =
    "butler-server";  // Folder name used as root dir
const std::string DEFAULT_SERVER_NAME =
    "localhost";  // Name of the server for HOST checking
...
server::UniquePtr<BIO> operator|(server::UniquePtr<BIO> lower,
                                 server::UniquePtr<BIO> upper) {
  BIO_push(upper.get(), lower.release());
  return upper;
}

// Struct to hold request information which needs to be passed around to fulfill
// the request
struct requestInfo {
  std::string method;
  std::string resource;
  std::string params;  // Params from request-target
...
class httpException : public std::exception {
 ...
 public:
  ...
  [[nodiscard]] const std::string &GetMErrMsg() const {
